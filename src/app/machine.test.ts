...
import { CAPTURE_SEQUENCE, type FaceLetter, type FaceCapture, type FaceLabels } from '../lib/cube';
...
describe('app machine', () => {
  ...
  it('capturing all six faces lands on review', () => {
  ...
  it('RESCAN_FACE jumps to that face in scan, preserving other captures', () => {
    let s = reducer(initialState, { type: 'START' });
    s = captureAll(s);
    s = reducer(s, { type: 'RESCAN_FACE', face: 'B' });
    expect(s.screen).toBe('scan');
    expect(CAPTURE_SEQUENCE[s.scanIndex].face).toBe('B');
    expect(Object.keys(s.labels)).toHaveLength(6); // others preserved
  });
